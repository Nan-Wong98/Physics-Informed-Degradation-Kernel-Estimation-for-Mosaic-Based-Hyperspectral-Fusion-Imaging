...
def visual_rgb(args):
    ...
    for file in tqdm.tqdm(files):
        ...
        if mat.dtype == numpy.int16:
            ...
            elif args.visual_task == "rgb_video":
                rgb = numpy.clip(numpy.round(mat / 2**12 * 255), 0, 255)
        else:
...
def main(args):
    if args.visual_task == "rgb" or "rgb_video":
        visual_rgb(args)
    elif args.visual_task == "diffmap":
        visual_diffmap(args)
    else:
        raise ValueError(f"Incorrect `visual_task`: {args.visual_task}. Set to `rgb`, or `diffmap`, or `rgb_video`  instead")
