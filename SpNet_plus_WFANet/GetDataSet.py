    ...
    def __init__(self, args, type="train"):
        ...
        if not os.path.exists(cache_path):
            ...
            for hrms_name in tqdm.tqdm(hrms_imgs):
                ...
                mosaic_noise_reshape = utils.MSFA_filter_inv(mosaic_noise, MSFA)
                mosaic_noise_reshape = mosaic_noise_reshape[:mosaic_noise_reshape.shape[0]//(args.msfa_size*args.spatial_ratio)*(args.msfa_size*args.spatial_ratio),
                                                :mosaic_noise_reshape.shape[1]//(args.msfa_size*args.spatial_ratio)*(args.msfa_size*args.spatial_ratio)]
    
                mosaic_noise_down = utils.MSFA_filter(mosaic_noise_reshape, MSFA)

                if type == "train":
                    mosaic_noise_down_patches = crop_to_patch(mosaic_noise_down, args.train_size, args.stride)
                    mosaic_noise_patches = crop_to_patch(mosaic_noise_reshape, args.train_size, args.stride)

                    self.mosaic += mosaic_noise_down_patches
                    self.target += mosaic_noise_patches
                elif type == "test":
...
class MakeRealWorldDatasetforDemosaic(Dataset):
        ...
        if not os.path.exists(cache_path):
            ...
            for mosaic_name, pan_name in tqdm.tqdm(tzip(mosaic_imgs, pan_imgs)):
                with open(os.path.join(mosaic_path, mosaic_name), "rb") as f:
                    ...
                    mosaic = numpy.zeros((255, 276, 16))
                    for i in range(args.msfa_size):
                        for j in range(args.msfa_size):
                            mosaic[:, :, i*4+j] = mosaic_raw[255*276*(i*args.msfa_size+j): 255*276*(i*args.msfa_size+j+1)].reshape(255, 276)
                ...
                mosaic_detach = mosaic[:mosaic.shape[0]//MSFA.shape[0]*MSFA.shape[0], :mosaic.shape[1]//MSFA.shape[1]*MSFA.shape[1], :]
                mosaic_down = utils.MSFA_filter(mosaic_detach, MSFA)

                if type == "train":
                    mosaic_down_patches = crop_to_patch(mosaic_down, args.train_size, args.stride)
                    mosaic_patches = crop_to_patch(mosaic_detach, args.train_size, args.stride)

                    self.mosaic += mosaic_down_patches
                    self.target += mosaic_patches

                elif type == "test":
                    self.mosaic.append(mosaic_down)
                    self.target.append(mosaic_detach)
    ...
    def __getitem__(self, index):
        ...
        target = torch.from_numpy(self.target[index].astype(numpy.float32)).permute(2, 0, 1)
        return mosaic, target
    ...
    def __init__(self, args, type="train", demosaic_net=None):
        ...
        if not os.path.exists(cache_path):
            ...
            for hrms_name in tqdm.tqdm(hrms_imgs):
                ...
                mosaic_tensor = torch.from_numpy(mosaic_noise.astype(numpy.float32)).permute(2, 0, 1).unsqueeze(0)
                with torch.no_grad():
