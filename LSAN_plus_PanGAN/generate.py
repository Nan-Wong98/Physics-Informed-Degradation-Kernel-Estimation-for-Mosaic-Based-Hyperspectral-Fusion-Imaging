...
def main(args):
    ...
    for cnt, (idx, mosaic, pan) in enumerate(tqdm.tqdm(tzip(ids, mosaics, pans))):
        if args.simulate == False:
            mosaic_tensor = torch.from_numpy((mosaic/2**12).astype(numpy.float32)).permute(2, 0, 1).unsqueeze(0).to(device)
            pan_tensor = torch.from_numpy((pan/2**12).astype(numpy.float32)).permute(2, 0, 1).unsqueeze(0).to(device)
        else:
