...
def main(args):
    ...
    if args.load_ps_model == "" or args.load_ps_model == None:
        ...
        ps_net.load_state_dict(torch.load(args.load_ps_model, map_location="cpu"), strict=False)
    ...
    for cnt, (idx, mosaic, pan) in enumerate(tqdm.tqdm(tzip(ids, mosaics, pans))):
        with torch.no_grad():
            if args.simulate == False:
                demosaic = PPID.PPID(copy.deepcopy(mosaic/2**12), MSFA) 
            ...
            demosaic_tensor = torch.from_numpy(demosaic.astype(numpy.float32)).permute(2, 0, 1).unsqueeze(0).to(device)
            upms_tensor = torch.nn.functional.interpolate(demosaic_tensor, scale_factor=args.spatial_ratio, mode="bicubic")

            hrms_tensor = utils.generate_patch(demosaic_tensor, pan_tensor, ps_net, size=64, recon_size=32, ratio=2).detach()[0].cpu()
            if args.simulate == False:
