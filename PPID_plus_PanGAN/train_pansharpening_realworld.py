...
from model import Generator, Discriminator_spe, Discriminator_spa, hp
import copy
import argparse
import json
import cv2
...
def main(args):
    ...
    train_set = MakeRealWorldDatasetForPansharpening(args, "train")
    train_dataloader = DataLoader(dataset=train_set, batch_size=args.batch_size, shuffle=True,
                                  num_workers=args.num_workers, pin_memory=True)
    val_set = MakeRealWorldDatasetForPansharpening(args, "test")
