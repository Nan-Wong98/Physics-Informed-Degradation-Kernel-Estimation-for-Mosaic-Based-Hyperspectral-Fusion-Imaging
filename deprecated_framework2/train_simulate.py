...
class Degrade_DMK(nn.Module):
    ...
    def forward(self, x, msfa_kernel, mosaic_flag=True):
        if x.shape[1] > 1:
            z = []
...
def train(fuse_net: nn.Module, demosaic_net: nn.Module, degrade_dmk: nn.Module, degrade_r: nn.Module, optimizers, train_dataloader, val_dataloader, args):
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        for cnt, data in enumerate(tqdm.tqdm(train_dataloader)):
            ...
            z_degrade = torch.nn.functional.conv2d(xlr_first, msfa_kernel_for_demosaic, stride=msfa_kernel_for_demosaic.shape[-1], groups=xlr_first.shape[1])
            ...
            loss_ei = torch.nn.functional.mse_loss(xlr_first, xlr_second)

            yDMK = degrade_dmk(y, msfa_kernel, mosaic_flag=False)

            loss_eq = torch.nn.functional.mse_loss(Rxlr, yDMK)
            ...
            loss =  loss_mosaic + loss_pan + loss_ei
            loss.backward()
            optimizers[1].step()
            optimizers[2].step()
            optimizers[3].step()
