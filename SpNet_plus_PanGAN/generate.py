from model import SpNet, Generator
import torch
import os
import numpy
...
def main(args):
    ...
    demosaic_net, ps_net = SpNet(args), Generator(args)
    ...
    for cnt, (idx, mosaic, pan) in enumerate(tqdm.tqdm(tzip(ids, mosaics, pans))):
        if args.simulate == False:
            mosaic_tensor = torch.from_numpy(mosaic.astype(numpy.float32)/2**12).permute(2, 0, 1).unsqueeze(0).to(f"cuda:{args.device}")
            pan_tensor = torch.from_numpy(pan.astype(numpy.float32)/2**12).permute(2, 0, 1).unsqueeze(0).to(f"cuda:{args.device}")
        else:
            mosaic_tensor = torch.from_numpy(mosaic.astype(numpy.float32)).permute(2, 0, 1).unsqueeze(0).to(f"cuda:{args.device}")
            pan_tensor = torch.from_numpy(pan.astype(numpy.float32)).permute(2, 0, 1).unsqueeze(0).to(f"cuda:{args.device}")
        with torch.no_grad():
